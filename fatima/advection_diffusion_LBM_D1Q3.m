% February 11th 2022 by Fatima Ezahra Chrit
% D1Q3 Lattice Boltzmann code for the case : advection-diffusion equation
...
flGaussianIni = 0;  %flag to solve ADE with gaussian initialization
...
%params for gaussian initialization
sigma0 = (L)/10;
mu0 = ceil(L/2) ;%12;

%D1Q3 lattice constants
w = [2/3 1/6 1/6]; % weight coefficients
...
D = 0.15; %0.00043;  % diffusion coefficient (equivalent to viscosity)
tau = D/csq + 1/2; %relaxation time
omega = 1/tau;
maxT  = 10; % total number of iterations 
...
if (flHeatDiffusion)
    ...
    if (flGaussianIni)
        C0 = 1; %1/(sigma0*sqrt(2*pi));
%         rho = C0 * exp(-(x-mu0).^2 / (2*sigma0^2));
...
figure; plot(x,rho,'*-r'); title('Solution  t = 0');
xlabel('x', 'FontSize', 16); ylabel('\rho', 'FontSize', 16);
% set(gca,'FontSize',16);
% return;
...
for cycle = 1:maxT
    ...
    % STREAMING STEP 
    f(1,:) = fout(1,:);
    f(2,2:nx) = fout(2,1:nx-1);
    f(3,1:nx-1) = fout(3,2:nx);
    
    if (flHeatDiffusion)
        %Heat diffusion BC
        f(2,1) = Tw - f(1,1) - f(3,1);       %Dirichlet BC
%         f(1,nx) = f(1,nx-1);        %zero flux
        f(2,nx) = f(2,nx-1);        %zero flux
        f(3,nx) = f(3,nx-1);        %zero flux
    else
        % periodic BC
        f(2,1) = f(2,nx);
        f(3,nx) = f(3,1);
...
%   analytical solution
    if (flHeatDiffusion)
        ...
        if(flGaussianIni)
            sigmaD = sqrt(2*D*cycle);
            rhoAnalytical = 1/4 * sigma0 / sqrt(sigma0^2 + 4*D*(cycle)) * exp(-(x-mu0 - ux*cycle).^2 / (sigma0^2 + 4*D*(cycle))) + 1/2;
        else
            rhoAnalytical = 1/sqrt(4*pi*D*cycle) * exp(- (x-mu0- ux*cycle).^2 / (4*D*cycle)) ;
        end
    end
...
%     % VISUALIZATION
    if (mod(cycle,tPlot)==0)
        plot(x,rho);
        hold on;
%         if(flHeatDiffusion || flGaussianIni)
            hold on
...
set(gca,'FontSize',16);
title(['Solution at t = ', num2str(maxT)]);
